...
<script type="text/javascript">
	function doFinalPartition() {
		window.open("http://localhost:8080/ICLD_PrivacyProtec/ShowPartition?method=doFinalPartition");
}
</script>
<%
	String[] FinalNSAs = (String[]) request.getAttribute("FinalNSAs");
	List<BlockInfo> BlockSetList = (List<BlockInfo>) request
			.getAttribute("BlockSetList");
	Long runningTime = (Long) request.getAttribute("runningTime");
%>
<body>
<button onclick="javascript:doFinalPartition();">进行φ2层划分</button>
	<table border="1" cellspacing="0">
		...
		<%
			int columnnum = 1;
			if (BlockSetList != null && BlockSetList.size() > 0) {
				for (BlockInfo blockinfo : BlockSetList) {
					int blocknum = 0;
					for (Map<String, Object> temp : blockinfo.getBlock()) {
		%>
		<tr>
			<%
				for (String obj : FinalNSAs) {
		...
		<tr>
			...
			<td>----</td>
			<td>----</td>
			<td>块划分到的层:<%=blockinfo.getLevel() %></td>
